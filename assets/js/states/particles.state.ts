...
let _materials: PointsMaterial[] = [];
let _particles: Points[] = [];
...
const setParticles = () => {
  ...
  for (let i = 0; i < PARTICLE_GROUP_SETTINGS.length; i += 1) {
    ...
    _materials[i] = new PointsMaterial({ size });

    const points = new Points(geometry, _materials[i]);
...
const setPointsRotation = (points: Points, rotation: number) => {
